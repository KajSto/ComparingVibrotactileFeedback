            ...
            lock (m_frameDataUpdateLock)
            {
                ...
                foreach (KeyValuePair<Int32, OptitrackMarkerState> markerEntry in m_latestMarkerStates)
                {
                    ...
                    else
                    {
                        ...
                        sphere.tag = "OptitrackMarker"; //ADDED BY KAJ --> to make markers easily found by Unity Editor
                        sphere.layer = LayerMask.NameToLayer("VRIgnore"); //ADDED BY KAJ --> to hider optitrack in VR
